...
public static class ServiceConnectActivitySource
{
        ...
        ActivityContext parentContext = default)
    {
        ...
        if (activity is null)
        {
            ...
            TraceContextPropagation.InjectTraceContext(Activity.Current, eventArgs.Headers);
            return null;
        }
        ...
        try
        {
            ...
            TraceContextPropagation.InjectTraceContext(activity, eventArgs.Headers);
        ...
        IMessagingSystemAttributes attributes)
    {
        ...
        DistributedContextPropagator.Current.ExtractTraceIdAndState(eventArgs.Headers, TraceContextPropagation.ExtractTraceIdAndState, out string? traceId, out string? traceState);
        ...
        if (!ActivityContext.TryParse(traceId, traceState, out ActivityContext parentContext))
        {
            ...
            malformedTraceparent = traceId is not null || TraceContextPropagation.HasTraceparentHeader(eventArgs.Headers);
    ...
    // Test seam forwarders so existing tests don't need to rewrite call sites.
        ...
        bool forceFreshRoot = false)
    {
        ...
        {
            parentContext = TraceContextPropagation.TryResolveFallbackParentContext();
        }
