...
namespace R.MessageBus.UnitTests.Stream
{
    public class ProcessStreamMessageTests
    {
        ...
        public void AfterStartMessageHasBeenProcessedAResponseShouldBeSentBackToTheSource()
        {
            ...
            _mockProducer.Setup(x => x.Send("Source", typeof(StreamResponseMessage).FullName, It.IsAny<byte[]>(), It.Is<Dictionary<string, string>>(y => y["ResponseMessageId"] == "MessageId")));
            ...
            // Assert
            _mockProducer.Verify(x => x.Send("Source", typeof(StreamResponseMessage).FullName, It.IsAny<byte[]>(), It.Is<Dictionary<string, string>>(y => y["ResponseMessageId"] == "MessageId")), Times.Once);
        }
