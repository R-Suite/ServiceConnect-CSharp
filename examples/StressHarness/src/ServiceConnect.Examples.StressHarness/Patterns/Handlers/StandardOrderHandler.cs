...
/// <summary>
/// Receives <see cref="StandardOrder"/> on either bus, records the handler arrival into
/// <see cref="FlowAccounting"/>, and signals the awaiting driver. Distinct from
/// <see cref="PremiumOrderHandler"/> so the driver can verify that the type-derived
...
/// </para>
/// </remarks>
public sealed class StandardOrderHandler(string busTag, FlowAccounting accounting, PerHandlerSignal signals, MessageLedger ledger, IChaosClock chaosClock)
    : IMessageHandler<StandardOrder>
{
    public Task HandleAsync(StandardOrder message, IConsumeContext context, CancellationToken cancellationToken = default)
    {
