...
- `headers` — optional string-valued transport headers. Reserved transport headers (`DestinationAddress`, `MessageId`, `MessageType`, `TypeName`, `FullTypeName`) are always overwritten by the implementation and must not be smuggled here.
- `cancellationToken` — cancels the in-progress send.
