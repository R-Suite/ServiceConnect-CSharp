...
1. Build an `Envelope` from the raw bytes and headers.
2. Run `IMessageProcessor` entries where `RunBeforeDeserialization == true`. If any returns `ProcessResult.Handled`, short-circuit.
...
5. Deserialise the message body via `IMessageSerializer`.
6. Run `IMessageProcessor` entries where `RunBeforeDeserialization == false` through the `IMessageProcessingMiddleware` chain.
7. Run after-consuming filters in a `finally` block so they fire even on failure.
8. Return a `ConsumeEventResult` describing the outcome.
...
1. Build `Envelope`.
2. Pre-deserialization processors (`RunBeforeDeserialization == true`) — short-circuit on `ProcessResult.Handled`.
3. Type resolution — return `Success = false` for unknown, non-reply types.
4. Before-consuming filters — short-circuit with `Success = true` on `false`.
5. Deserialise.
6. Post-deserialization processors (`RunBeforeDeserialization == false`) through the middleware chain.
7. After-consuming filters in `finally`.
8. Return `ConsumeEventResult`.
...
### Tracing dispatcher with per-phase Activity spans

The following example wraps the `IMessageDispatcher` call and emits a root `Activity` for the entire consume pipeline plus child spans for each phase. Because it replaces the default dispatcher it must also re-implement the pipeline; in practice, prefer delegating to the default dispatcher if all you need is the root span.
...
public sealed class TracingMessageDispatcher : IMessageDispatcher
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            using (Source.StartActivity("resolve"))
            using (Source.StartActivity("filter"))
            using (Source.StartActivity("deserialize"))
            using (Source.StartActivity("dispatch"))
            {
