...
public sealed class StartupValidatingHandlerRegistry : IHandlerRegistry
{
    ...
    {
        // Verify that handlers for the expected message types are registered.
        AssertHandlerPresent<OrderPlaced>(services);
        AssertHandlerPresent<PaymentAuthorised>(services);
    }

    private static void AssertHandlerPresent<TMessage>(IServiceProvider services)
    {
        var handler = services.GetService<IMessageHandler<TMessage>>();
        if (handler is null)
            throw new InvalidOperationException(
                $"No IMessageHandler<{typeof(TMessage).Name}> is registered. " +
                "Ensure the handler is added before AddServiceConnect is called.");
    }
}
```
