...
- `type` — receives the resolved `Type` if the method returns `true`; the value is undefined when the method returns `false`.
...
<Aside type="caution">
The `out` parameter is declared as `out Type` (non-nullable) to match the source interface. An implementation must still assign a value to the parameter in the `false` branch — assign `type = null!` or `type = typeof(object)` as a placeholder when returning `false`.
</Aside>
