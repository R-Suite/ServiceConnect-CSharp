...
**Remarks.** Shorter intervals reduce latency between scheduled delivery and actual delivery at the cost of more database round-trips. A few seconds is typical. **Default:** 30 seconds.
...
**Remarks.** Higher values improve throughput for handlers that spend time in I/O; they do not improve throughput for CPU-bound handlers and increase memory footprint per consumer. **Default:** 1.
