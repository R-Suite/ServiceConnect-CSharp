...
Sends `message` to every endpoint in `endPoints` as an independent point-to-point delivery. Replaces the v7 `SendOptions.EndPoints` plural pattern — fan-out is now an explicit per-call list rather than a property on the options struct.
...
<Aside type="note" title="Not every implementation supports this">
  The default interface-method implementation returns `Task.FromException(new NotSupportedException(...))` — the exception is deferred into the returned task rather than thrown synchronously. A concrete bus must be paired with a registered `ITimeoutStore` (for example via `UseMongoDbPersistence`) and have `IBusConfiguration.EnableProcessManagerTimeouts` set to true for this call to succeed.
</Aside>
