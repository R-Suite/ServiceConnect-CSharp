...
- `options` — optional headers and a routing-key override (`PublishOptions`).
...
- `options` — an optional destination endpoint override and headers (`SendOptions`).
...
<Aside type="note" title="Multi-endpoint Send">
  When `Send` resolves to multiple endpoints — through [`SendToManyAsync`](#sendtomanyasynct) or because [`AddQueueMapping`](../../configuration/iqueueconfiguration/#addqueuemapping) registered several queues for the message type — each delivery is an independent on-wire message with a distinct `MessageId` and `TimeSent`. Use `CorrelationId` to correlate deliveries that originated from the same logical bus operation; it remains constant across the fan-out.

  Failure semantics: see the [`IProducer.SendAsync` (auto-routed) failure-semantics block](../../extension-points/transport/iproducer/#sendasync-auto-routed) — per-endpoint failure aggregates into `AggregateException`, cancellation aggregates with prior failures (or propagates plain if no prior failures exist).
</Aside>
...
**Throws**
- `ArgumentNullException` — `message` or `endPoints` is `null`.
- `ArgumentException` — `endPoints` is empty.
- `OutgoingFiltersBlockedException` — an outgoing filter returned `FilterAction.Stop` before the message reached the transport.
...
- `ArgumentNullException` — `message` is `null`.
- `ArgumentOutOfRangeException` — `options.Timeout` is negative (other than `Timeout.Infinite`, the `int` constant `-1`) or zero.
- `ObjectDisposedException` — the bus has been disposed.
...
- `ArgumentNullException` — `message` or `onReply` is `null`.
- `ArgumentException` — `options.EndPoint` is non-empty; use `SendRequestAsync` for single-destination requests.
- `ArgumentOutOfRangeException` — `options.Timeout` is negative (other than `Timeout.Infinite`, the `int` constant `-1`) or zero.
- `ObjectDisposedException` — the bus has been disposed.
- `OutgoingFiltersBlockedException` — an outgoing filter returned `FilterAction.Stop`.
...
**Throws**
- `ArgumentNullException` — `message` or `destinations` is `null`.
- `ArgumentException` — `destinations` is empty or contains an entry with a comma (the routing-slip separator) or that otherwise fails destination validation.
- `ObjectDisposedException` — the bus has been disposed.
- `OutgoingFiltersBlockedException` — an outgoing filter returned `FilterAction.Stop` before the message reached the transport.
...
<Aside type="caution" title="Destination validation">
...
**Throws**
- `ArgumentException` — `endpoint` is `null` or whitespace.
- `ObjectDisposedException` — the bus has been disposed.
...
**Throws**
- `ArgumentException` — `correlationId` is `Guid.Empty`.
- `ArgumentOutOfRangeException` — `delay` is less than or equal to `TimeSpan.Zero`.
...
- `NotSupportedException` — the bus implementation does not support scheduling timeouts; the default-interface-method path defers it into the returned task rather than throwing synchronously (see the Aside below).
- `ObjectDisposedException` — the bus has been disposed.

<Aside type="note" title="Not every implementation supports this">
  The default interface-method implementation returns `Task.FromException(new NotSupportedException(...))` — the exception is deferred into the returned task rather than thrown synchronously. A concrete bus must be paired with a registered `ITimeoutStore` (for example via `UseMongoDbPersistence`) for this call to succeed. `IBusConfiguration.EnableProcessManagerTimeouts` is a separate concern: it controls whether the polling hosted service dispatches scheduled timeouts back into the handler pipeline. The scheduling call itself only requires an `ITimeoutStore`; if the flag is off the timeout still inserts but nothing fires it.
</Aside>
