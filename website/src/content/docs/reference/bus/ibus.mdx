...
<Aside type="note" title="Multi-endpoint Send">
  When `Send` resolves to multiple endpoints — either via [`SendOptions.EndPoints`](../../messages/options/#endpoints) or because [`AddQueueMapping`](../../configuration/iqueueconfiguration/#addqueuemapping) registered several queues for the message type — each delivery is an independent on-wire message with a distinct `MessageId` and `TimeSent`. Use `CorrelationId` to correlate deliveries that originated from the same logical bus operation; it remains constant across the fan-out.
