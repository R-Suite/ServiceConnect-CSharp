...
The concrete `PipelineConfiguration`, like the rest of the `*Configuration` concrete classes (`BusConfiguration`, `TransportConfiguration`, `QueueConfiguration`, `PersistenceConfiguration`), is `internal` from v7 onward. Only the `I*Configuration` interfaces are public — consumer code interacts with them via the builder callbacks (`ConfigureTransport`, `ConfigureQueues`, etc.) or by reading `IBusConfiguration.Pipeline` (or other sub-config interfaces) from DI for inspection. Constructing a `new PipelineConfiguration()` directly is no longer supported from outside the framework assembly.
