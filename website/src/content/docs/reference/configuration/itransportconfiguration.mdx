...
```csharp
transport.SetClientSetting(RabbitMQSettingKeys.MaxHeaderValueBytes, 16 * 1024);
```

Caps the bytes any single header value may carry on an inbound message. Accepts a positive `int`; defaults to **8192** (8 KB) when unset. Equivalent to `RabbitMqOptions.MaxHeaderValueBytes` on the typed overload.
