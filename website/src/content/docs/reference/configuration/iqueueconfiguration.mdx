...
**Remarks.** This is the inbox consumed by `IBus.StartConsumingAsync` and the `SourceAddress` header stamped on outgoing messages. An empty value causes consumption to fail at startup.
...
**Remarks.** Only populated when [`AuditingEnabled`](#auditingenabled) is true. Interpreted by RabbitMQ as the target exchange; audit publishes always use an empty routing key (see [`AuditRoutingKey`](#auditroutingkey)).
...
**Reserved — currently ignored.** The RabbitMQ audit publisher always publishes with `routingKey=""` because the audit direct exchange is bound to the audit queue with an empty routing key; any non-empty value configured here would be unroutable (`mandatory=false`) and silently dropped.

**Remarks.** The property is retained for forward compatibility; setting it to a non-empty value causes `MessageAuditPublisher` to log a Warning at construction and still publishes with an empty routing key. Leave the default empty string.
