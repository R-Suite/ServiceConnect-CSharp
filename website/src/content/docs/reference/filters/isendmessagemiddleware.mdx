...
**Parameters**
- `context` — the `SendContext` describing the outgoing publish (see [`SendContext` properties](#isendmessagemiddleware) above). The bus populates `Operation = SendOperation.Publish` before invoking the pipeline.
