...
<Aside type="note" title="No ambient Stream property">
  Stream handlers receive the read stream as an explicit `IMessageBusReadStream stream` parameter on `HandleAsync`. There is no property setter the framework writes between dispatches.
</Aside>
