...
public sealed class QuoteShippingHandler : IMessageHandler<QuoteShipping>
{
    ...
    public async Task HandleAsync(QuoteShipping message, CancellationToken cancellationToken = default)
    {
        var ctx = Context!;

        // Priority is optional on the request; default to "standard" when absent.
        var priority = ctx.Headers.TryGetValue("ShippingPriority", out var raw)
            ? DecodeString(raw)
            : "standard";
        ...
        await ctx.ReplyAsync(new ShippingQuote
        {
