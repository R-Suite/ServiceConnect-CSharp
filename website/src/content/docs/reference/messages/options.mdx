...
**Remarks.** Default is `null`. Use this when a single logical send should be delivered to several queues in one call. Any `IReadOnlyList<string>` implementation — `List<string>`, an immutable list, an array — is assignable at construction time via the object initialiser.
...
**Remarks.** Default is `null`. Overrides the configured queue mapping for the request message type. Mutually exclusive with `EndPoints`.
...
Gets the destination endpoints when broadcasting a multi-request.

**Remarks.** Default is `null`. Used by `SendRequestMultiAsync` to fan out to several respondents. Mutually exclusive with `EndPoint`.
