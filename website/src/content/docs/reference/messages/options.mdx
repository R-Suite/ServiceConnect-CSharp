...
Gets a fresh `RequestOptions` populated with default values.

**Returns.** A new `RequestOptions` instance per access, so mutating the returned object does not leak into other callers using the fallback. Handy when you want to pass defaults explicitly rather than `null`.
