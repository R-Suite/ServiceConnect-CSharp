...
Registers `BusConsumingHealthCheck`. The default overload uses `ActivatorUtilities.CreateInstance<BusConsumingHealthCheck>` so `IBus` is resolved from the DI container at probe time. Use the factory or keyed-services overload when multiple named bus instances are registered in the same DI container.
...
- `HasAttemptedConnection` is `false` → returns `Healthy("Producer has not yet attempted a connection.")`. This is the state before any publish or send has been issued; treating it as unhealthy would crash-loop pods unnecessarily.
