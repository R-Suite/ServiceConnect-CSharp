...
**Returns.** The maximum aggregation window. The default returns `Timeout.InfiniteTimeSpan`, which disables the timeout-based flush (the dispatcher gates scheduling on a value strictly greater than `TimeSpan.Zero`). Returning any positive `TimeSpan` enables time-based flushing.
...
<Aside type="caution" title="Always set at least one flush trigger">
  If both `Timeout()` and `BatchSize()` return their defaults (`Timeout.InfiniteTimeSpan` and `0` respectively), the aggregator accumulates indefinitely and never flushes. Override at least one.
</Aside>
...
Gets the deserialised messages recovered from the persistor. Values are boxed to `object` because the snapshot spans heterogeneous record types; cast to `T` before handing the list to `Aggregator<T>.ExecuteAsync`.
