...
| `MessagingOperationName` | `messaging.operation.name` | `publish` for sends/publishes, `receive` for consumer-side handler dispatch. (Semconv 1.x.) |
...
| `ErrorType` | `error.type` | Mapped exception type for failure paths. Closed allow-list (`cancelled`, `timeout`, `forbidden`, …); never the exception message. |
