...
public static class ServiceConnectActivitySource
{
    ...
    public static bool IsConsumeTelemetryEnabled(
        ServiceConnectInstrumentationOptions options);
...
### `IsConsumeTelemetryEnabled`

Returns `options.EnableConsumeTelemetry && ActivitySource.HasListeners()`. Used internally by `TelemetryProcessingMiddleware` to gate the `envelope.Body.ToArray()` copy that feeds `EnrichWithMessageBytes` — the allocation is skipped entirely when no listener is attached or consume telemetry is disabled.
