...
builder.Services.AddServiceConnect(sc =>
{
    ...
    sc.AddHandlersFromAssemblyContaining<Program>();
    sc.AutoStartConsuming();
});
...
- `AutoStartConsuming()` replaces the explicit `bus.StartConsuming()` call.
...
`IProcessHandler<T>` and `IStreamHandler<T>` follow the same shape:

```csharp
Task IProcessHandler<TData, TMsg>.HandleAsync(
    TMsg message, IProcessHandlerContext<TData> ctx, CancellationToken ct);

Task IStreamHandler<T>.ExecuteAsync(
    IMessageBusReadStream stream, IConsumeContext ctx, CancellationToken ct);
```
...
```csharp
await bus.SendToManyAsync(new ChargeCard(), new[] { "payments-a", "payments-b" }, ct);
...
```csharp
public class MessageProcessedRecorder : IConsumeMessageFilter
{
    ...
    public async Task<bool> ProcessAsync(IConsumeContext ctx, CancellationToken ct)
    {
        await store.RecordAsync(ctx.MessageId, ct);
        return true;
    }
}
...
A reference implementation lives in [`examples/CustomFilterAndMiddleware`](https://github.com/R-Suite/ServiceConnect-CSharp/tree/v7-clean-architecture/examples/CustomFilterAndMiddleware).
...
- `messaging.operation` (the pre-1.x string attribute) is **no longer emitted**. Replace dashboard filters with `messaging.operation.type` (`publish` / `process`) and `messaging.operation.name` (`publish` / `receive`).
...
There's also only one `ActivitySource` now: `"ServiceConnect.Bus"`. OTel listeners that subscribed to the three pre-v7 sources should collapse to a single `AddSource("ServiceConnect.Bus")` call.
...
4. **Sanity-check your OTel pipeline.** Confirm the dashboards you rely on still light up under the new attribute names and the single `ServiceConnect.Bus` source.
