...
- **Both size and time are optional knobs.** Return `0` from `BatchSize()` to disable size-based flushing; leave the default `Timeout.InfiniteTimeSpan` from `Timeout()` to disable time-based flushing. Whichever you override with a positive value is the flush trigger. Setting both — as above — flushes on whichever happens first.
