...
- **The generic parameter names the handler target.** The generic parameter `<DocumentUploaded>` names the message type the receiver's handler will be dispatched for. The bytes written into the stream are deserialised as `DocumentUploaded` on the receiver — that is the message the handler receives. No separate message instance is sent alongside the bytes.
