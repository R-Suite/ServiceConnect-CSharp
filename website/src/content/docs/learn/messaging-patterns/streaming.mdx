...
public sealed class DocumentUploadedHandler : IStreamHandler<DocumentUploaded>
{
    ...
    public void Execute(DocumentUploaded message)
    {
        ...
        File.WriteAllBytes(message.FileName, bytes);
    }
}
```

- **`Stream` is a property, not a parameter.** The dispatcher sets it before calling `Execute`. This is the same pattern as `IConsumeContext` on a normal handler — framework-populated.
- **`Execute` is synchronous.** By the time it is called, every packet has arrived and been reassembled in memory; the I/O-bound wait is over. If the work inside `Execute` is itself async, run it on a background task or block — but *don't* make that work slow, because it serialises against the next stream's reassembly.
- **`Stream.Read()` returns the whole byte array.** The reassembly is complete by the time the handler fires; no packet-by-packet processing is expected. For very large payloads where holding everything in memory is painful, `Stream.ReadSequence()` returns a `ReadOnlySequence<byte>` you can stream through.
...
The receiver reassembles in memory. A 5 GB payload will hold 5 GB in the receiver's heap until `Execute` returns. If that is not acceptable, chunking the *semantics* (many small `DocumentChunkUploaded` messages with sequence numbers) is the right move, not streaming.
