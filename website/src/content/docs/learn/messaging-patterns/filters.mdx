...
- **`FilterAction.Stop`** — no further filters run on this envelope, and the message is dropped.
...
The *before* position is where "should this message even be processed?" goes — auth, deduplication, feature-flag gates. Returning `FilterAction.Stop` here drops the message cleanly: the handler never runs, and the pipeline treats it as successfully processed (so the message is acked and does not go to the error queue).
