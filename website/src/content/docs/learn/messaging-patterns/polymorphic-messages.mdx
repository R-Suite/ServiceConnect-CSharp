...
- **Subscription does not walk the hierarchy.** Each `HandlerReference` binds the subscriber's queue to exactly one RabbitMQ exchange — the one derived from that `MessageType` (a sanitised type name plus a short hash suffix; [as described on the Pub/Sub page](/ServiceConnect-CSharp/learn/messaging-patterns/pub-sub/#how-the-fan-out-works)). The publisher emits `OrderPlaced` to the `OrderPlaced` exchange; if nothing bound the audit queue to that exchange, the message never arrives.
