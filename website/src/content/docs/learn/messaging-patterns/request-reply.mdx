...
The request methods raise `RequestSendCancelledException` (inheriting from `OperationCanceledException`) when the outbound send pipeline cancels before the request reaches the broker — distinct from a timeout (`RequestTimeoutException`) and from caller-token cancellation (`OperationCanceledException`). It applies to `SendRequestAsync`, `SendRequestMultiAsync`, and `PublishRequestAsync` alike.
