...
- **Transport** — which broker the bus talks to. `UseRabbitMQ` is the supported transport in v7.
...
The caller `await`s the reply. Under the hood, ServiceConnect correlates the reply using the `ResponseMessageId` header, which the reply handler sets to the value of the outbound `RequestMessageId`.
