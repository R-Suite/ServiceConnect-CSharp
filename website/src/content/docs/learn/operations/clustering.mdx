...
| `HeartbeatTime` | 60s | AMQP heartbeat interval. Disabling heartbeats (`HeartbeatEnabled = false`) removes broker-side dead-peer detection and is rarely what you want in a clustered deployment. |

Both are set via the `UseRabbitMQ(opts => ...)` typed options surface — see [RabbitMqOptions](https://github.com/ServiceConnect/ServiceConnect-CSharp/blob/master/src/ServiceConnect.Client.RabbitMQ/Configuration/RabbitMqOptions.cs) for the full set.
