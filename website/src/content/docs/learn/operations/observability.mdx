...
| 4 | `ConnectionRecovered` | Information | RabbitMQ.Client's auto-recovery restores the **connection** — topology (exchanges, queues, bindings) is not re-declared automatically (`TopologyRecoveryEnabled = false`); the application owns topology and declares it at startup |
