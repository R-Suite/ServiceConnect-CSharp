...
The **`ServiceConnect.Telemetry`** package provides the `ActivitySource` plumbing for distributed tracing. It exposes three sources — `ServiceConnect.Bus.Publish`, `ServiceConnect.Bus.Send`, and `ServiceConnect.Bus.Consume` — independently sampleable, each emitting spans tagged with the OTel messaging semantic conventions (`messaging.system`, `messaging.destination`, `messaging.message.id`, `messaging.message.conversation_id`). Pub/sub and point-to-point traffic are distinguished by the `messaging.operation` tag (`publish` vs `send`) so backends aggregate each shape separately.
...
End-to-end correlation across the broker is automatic: `ServiceConnectActivitySource.Publish` and `Send` inject the W3C `traceparent`/`tracestate` onto the outgoing headers, and `Consume` extracts them on the way in. A consume span on the far side of the broker is a direct child of the publish span, not a new root.

Wire the package into your bus by calling the `Publish`, `Send`, and `Consume` helpers from a filter or custom outgoing hook and passing the event-args exposed by the pipeline — see [`event-args`](/ServiceConnect-CSharp/reference/handlers/event-args/) for the DTO shapes. To enrich spans with decoded-message data or toggle a source, configure the static `ServiceConnectActivitySource.Options`:

```csharp
ServiceConnectActivitySource.Options.EnableSendTelemetry = true;
ServiceConnectActivitySource.Options.EnrichWithMessage = (activity, message) =>
