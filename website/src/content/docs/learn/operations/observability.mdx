...
Call `builder.AddTelemetry()` inside `AddServiceConnect`, then register the three activity sources with your OTel tracing builder:
...
All three sources are enabled by default. Disable any you don't need to keep sampling noise down:
...
builder.AddTelemetry(opts =>
{
    ...
    opts.EnableConsumeTelemetry = true;
});
```
