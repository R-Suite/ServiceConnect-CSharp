...
Framework exceptions live under `ServiceConnect.Interfaces.Exceptions`. They all derive from one abstract base, so a single catch can quarantine every framework-originated failure without swallowing user exceptions:
