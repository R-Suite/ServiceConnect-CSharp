...
`IBusConfiguration.ExceptionHandler` is a callback invoked on every dispatch-level exception. It runs alongside the retry machinery — not instead of it:

```csharp
builder.ConfigureBus(bus =>
{
    bus.ExceptionHandler = ex =>
    {
