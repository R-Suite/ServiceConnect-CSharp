...
- [ ] **Poison-message infinite redelivery when retry/error publish throws** — [RabbitMqConsumerHost.cs:166-227, 229, 252, 353-360](../src/ServiceConnect.Client.RabbitMQ/RabbitMqConsumerHost.cs)
  If `HandleFailureAsync` / `HandleTerminalFailureAsync` throws (broker outage, publish-channel closed), the outer catch nacks the original with `requeue:true`. Audit is guarded with its own try/catch; retry and terminal-failure are not. Under a partial broker outage the redelivery loop is reachable — question is whether the self-throttle via broker RTT is acceptable back-pressure or a real hot-loop.
  **Test to write (integration):** RabbitMQ Testcontainer; start a consumer with a handler that always throws; then simulate a retry-publish failure (e.g. close the retry exchange) and assert the message redelivery count bound over a fixed window. If redelivery count grows unbounded within the test window, the claim stands. [r4]
